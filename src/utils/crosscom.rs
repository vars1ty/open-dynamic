...
impl CrossCom {
    ...
    pub fn connect(&self, serial: &'static String) {
        ...
        listener.for_each(move |event| match event {
            NodeEvent::Network(net_event) => match net_event {
                ...
                NetEvent::Accepted(..) => unreachable!(), // Only generated by listening
                NetEvent::Message(_, data) => {
                    let Ok(server_data) = (unsafe {
                        rkyv::from_bytes_unchecked(&CompressionUtils::decompress(data))
                    }) else {
                        return;
                    };
                ...
                NetEvent::Disconnected(_) => {
                    // The session shouldn't be kept alive if we are disconnected.
    ...
    fn handle_server_data(&self, server_data: CrossComServerData) {
        match server_data.data_type {
            DataType::AuthSuccess => {
                self.send_data_type(DataType::GetVersion(None), None);
            }
    ...
    pub fn send_to_channel(&self, data: CrossComServerData) {
        let Some((sender, _)) = self.get_network_listener().get_crossbeam_channel().get() else {
            return;
        };
